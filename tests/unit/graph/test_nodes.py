...
from tests.fakes import ScriptedCaller, hypothesis_set, triage_answer, verdict
...
async def test_triage_disagreeing_with_the_signature_forces_an_investigation(
    ...
    updates = await TriageNode(caller)(_state(failure_event, budgets))

    assert updates["triage"].needs_investigation is True
    assert updates["triage"].confidence < 0.9


async def test_triage_tells_the_model_what_pattern_matched(toolbox, budgets, failure_event):
    caller = ScriptedCaller({"triage": [triage_answer()]})

    await TriageNode(caller)(_state(failure_event, budgets))

    assert "undefined_column" in caller.prompt_for("triage")


async def test_a_failure_with_no_exception_recorded_is_still_triaged(budgets, failure_event):
    caller = ScriptedCaller({"triage": [triage_answer(category=RootCauseCategory.UNKNOWN)]})
    bare = failure_event.model_copy(update={"exception_type": None, "exception_message": None})
...
async def test_testing_without_a_hypothesis_does_nothing_rather_than_failing(
    ...
    updates = await TestHypothesisNode(caller, toolbox)(_state(failure_event, budgets))
...
async def test_a_test_that_cannot_be_run_within_budget_says_so_rather_than_pretending(
    ...
    exhausted = _state(failure_event, budgets, tool_calls_made=1)
...
async def test_a_hypothesis_citing_evidence_that_is_not_there_is_kept_anyway(
    ...
    updates = await FormHypothesisNode(caller, toolbox)(_state(failure_event, budgets))
